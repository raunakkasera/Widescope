class BudgetsController < ApplicationController
 ...
 def show
  ...
  def create
    @budget_item = Budget.new(params[:budget_item])

    respond_to do |format|
      if @budget_item.save
        format.html { redirect_to(@budget_item, :notice => 'BudgetItem was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @budget_item.update_attributes(params[:budget_item])
        format.html { redirect_to(@budget_item, :notice => 'BudgetItem was successfully updated.') }
        format.xml  { head :ok }
      else
